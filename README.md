...
`web-extract` is a small Node library that fetches a web page and returns a token-budgeted, deterministic representation suitable for stuffing into an LLM context window. Ships with an MCP server so it's usable as a tool from Claude Desktop, Claude Code, or any MCP client without writing glue code.
...
```ts
import { extract, type FetchCache } from "web-extract";
